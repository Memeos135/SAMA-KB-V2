...
1. Introduction
1.1 
The Basel Committee on Banking Supervision issued the Basel III: Finalizing 
...
1.2 
The output floor requirements are issued by SAMA in exercise of the authority 
...
2.1 
These requirements apply to all domestic banks both on a consolidated basis, 
which include all branches and subsidiaries, and on a standalone basis.
2.2 
These requirements are not applicable to foreign banks’ branches operating in 
...
3.1 
These requirements will be effective on 1 January 2023, subject to the 
...
definitions in SAMA’s Enhanced Finalized Guidance Document Concerning the 
...
RWA for Credit Risk
5.4 
RWA for credit risk and counterparty credit risk is calculated as the sum of the 
following: 
...
(5) RWA for exposures to central counterparties in the banking book and trading
...
RWA for Operational Risk
5.6 
RWA for operational risk is calculated using the standardized approach for 
...
Calculation of the Output Floor
5.7 
The standardized approaches to be used to calculate the base of the output floor 
...
5.8 
As per paragraph 5.7 above, the following approaches are not permitted to be 
...
5.9 
SAMA may review the level of the incremental increase for all banks. In 
...
Phase-in arrangements for output floor 
Table 1 
Date 
Calibration 
1 January 2023 
